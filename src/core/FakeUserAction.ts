...
export class FakeUserAction {
   ...
   * @param options
   */
  private static async simMouseMove(
    page: Page,
    options: {
      startPos: Point;
      endPos: Point;
    ...
    for (let n = 0; n < points.length; n += 1) {
      ...
      await helper.sleep(
        (options.timestamp || helper.rd(300, 800)) / points.length
      );
    }
  }
  ...
  async simClick(
    ...
    if (fb.isMobileBrowser) {
      ...
      await currPage.touchscreen.tap(
        this._mouseCurrPos.x,
        this._mouseCurrPos.y
      );
    } else {
  ...
  async simMoveToAndClick(
    ...
    if (!fb.isMobileBrowser) {
      await this.simMouseMoveTo(endPos);
      await currPage.mouse.move(endPos.x + helper.rd(-10, 10), endPos.y, {
  ...
  async simMouseMoveToElement(eh: ElementHandle): Promise<boolean> {
    ...
    if (fb.isMobileBrowser) {
      box = await FakeUserAction.adjustElementPositionWithTouchscreen(
        eh,
        currPage,
        fakeDD
      );
    } else {
      box = await FakeUserAction.adjustElementPositionWithMouse(
        eh,
        currPage,
        fakeDD
      );
    ...
    for (;;) {
      ...
      if (box) {
        ...
        if (box.y <= 0) {
          ...
          deltaY = Math.min(
            box.y + box.height - fakeDD.window.innerHeight + 30,
            helper.rd(100, 300)
          );
          viewportAdjust = true;
        }
        ...
        if (viewportAdjust) {
          ...
          const _patchTouchscreenDesc = Object.getOwnPropertyDescriptor(
            currPage,
            "_patchTouchscreen"
          );
          ...
          const startX: number =
            fakeDD.window.innerWidth / 2 +
            helper.rd(0, fakeDD.window.innerWidth / 6);
          const endX: number =
            fakeDD.window.innerWidth / 2 +
            helper.rd(0, fakeDD.window.innerWidth / 6);
          let startY: number;
