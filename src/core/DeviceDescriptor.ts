...
export default class DeviceDescriptorHelper {
  ...
  static checkLegal(dd: DeviceDescriptor): boolean {
    ...
    if (!UserAgentHelper.isMobile(dd.navigator.userAgent)) {
      ...
      if (
        dd.window.innerHeight > dd.screen.availHeight ||
      ...
      if (
        !dd.plugins ||
        !dd.plugins.mimeTypes.length ||
        !dd.plugins.plugins.length
      ) {
  ...
  static deviceUUID(e: DeviceDescriptor): string {
    return crypto.createHash("md5").update(JSON.stringify(e)).digest("hex");
  }
  ...
  static buildFakeDeviceDescriptor(deviceDesc: DeviceDescriptor): {
    ...
    const fakeDD: FakeDeviceDescriptor = helper.objClone(
      deviceDesc
    ...
    if (!fakeDD.fontSalt) {
      ...
      const nonexistentFonts = extraFonts.filter(
        (e) => !existsFonts.includes(e)
      );
