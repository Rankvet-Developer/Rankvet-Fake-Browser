  ...
  static async patchServiceWorkerRequest(
    ...
    try {
      ...
      if (responseHeaders && responseHeaders.length) {
        let body: string;
        ({ body, base64Encoded } = await client.send("Fetch.getResponseBody", {
          requestId,
        }));
        jsContent = base64Encoded
          ? Buffer.from(body, "base64").toString("utf-8")
          : body;
      } else {
        // TODO: get through proxy
        const jsResp = await axios.get(request.url, {
          headers: request.headers,
        });
        jsContent = jsResp.data;

        responseHeaders = Object.entries(jsResp.headers).map((e) => ({
          name: e[0],
          value: e[1] as string,
        }));
      }
