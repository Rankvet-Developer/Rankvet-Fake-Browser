...
export class PptrToolkit {
  static async waitForSelectorWithRegex(
    ...
    for (;;) {
      const result = await this.querySelectorAllWithRegex(
        page,
        reg,
        attributeToSearch
      );
  ...
  static async stopLoading(page: Page) {
    try {
      await (page as any)._client().send("Page.stopLoading");
    } catch (ex: any) {}
  ...
  static async boundingBoxNew(eh: ElementHandle): Promise<{
    ...
    try {
      const { model } = await (eh as any)._client().send("DOM.getBoxModel", {
        objectId: (eh as any)._remoteObject().objectId,
      });
  ...
  static async boundingBox(
    eh?: ElementHandle | null
  ...
  static async intersectingViewport(
    ...
    if (
      box.y > 0 &&
      box.y + box.height < fakeDD.window.innerHeight &&
      box.x > 0 &&
      box.x + box.width < fakeDD.window.innerWidth
    ) {
      return box;
    }
