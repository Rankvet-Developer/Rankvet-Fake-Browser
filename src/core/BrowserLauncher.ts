...
import { PptrPatcher } from "./PpptrPatcher";

const kFakeDDFileName: string = "__fakebrowser_fakeDD.json";
const kInternalHttpServerHeartbeatMagic: string = "__fakebrowser__&88ff22--";
...
export class BrowserLauncher {
  ...
  private static checkLaunchOptionsLegal(options?: VanillaLaunchOptions) {
    ...
    if (
      options.args.filter((e) =>
        externalCannotSetArgs.includes(e.toLocaleLowerCase().split("=")[0])
      ).length > 0
    ) {
      throw new TypeError(
        `${externalCannotSetArgs} cannot be set in options.args`
      );
    }
  }
  ...
  private static prepareFakeDeviceDesc(params: DriverParameters) {
    ...
    const { fakeDeviceDesc, needsUpdate } =
  ...
  static async launch(params: LaunchParameters): Promise<FakeBrowser> {
    ...
    const { vanillaBrowser, pptrExtra } = await Driver.launch(
      uuid,
    ...
    if (!this._httpServer) {
        ...
        async (req: IncomingMessage, res: ServerResponse) => {
          ...
          if (pathname === "/patchWorker") {
            ...
            const reqHeaders = Object.fromEntries(
              Object.entries(req.headers).map((e) => [e[0], e[1]![0]])
            );
            ...
            if (browser) {
              jsContent = await PptrPatcher.patchWorkerJsContent(
                browser,
                jsContent
              );
            }
      ...
      try {
        this._httpServer.listen(
          FakeBrowser.globalConfig.internalHttpServerPort
        );
      } catch (ex: any) {
      ...
      this._checkerIntervalId = setInterval(async () => {
        const killThese = this._fakeBrowserInstances.filter(
          (e) =>
            ...
            new Date().getTime() >
              e.bindingTime + e.launchParams.maxSurvivalTime
        );
