...
function getUserAgent(): string {
    const data = fs.readFileSync(
        path.join(__dirname, './useragent/filter.txt'),
        'utf-8',
    );

    const allUserAgent = data.split('\n');
    const index = _rd(0, allUserAgent.length - 1);
    const ua = allUserAgent[index];
    return ua;
}
