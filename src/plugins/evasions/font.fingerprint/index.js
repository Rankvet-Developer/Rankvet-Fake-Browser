...
class Plugin extends PuppeteerExtraPlugin {
    ...
    mainFunction = (utils, fontSalt) => {
        ...
        function parseFont(value) {
            ...
            function stringify(arg, opts) {
                if (opts && opts.flat) {
                    ...
                    while (str != prevStr) {
                        if (a++ > 10e3)
                            throw Error('Circular references in ' + arg);
                        prevStr = str;
                ...
                function replaceRef(match, idx) {
                    if (arg[idx] == null)
                        throw Error('Reference ' + idx + 'is undefined');
                    return arg[idx];
                }
            }
            ...
            function splitBy(string, separator, o) {
                ...
                if (separator == null)
                    throw Error(
                        'Separator should be a string or a RegExp',
                    );
                ...
                const tokens = paren.parse(string, {
                    flat: true,
                    brackets: o.ignore,
                });
                const str = tokens[0];
            ...
            while ((token = tokens.shift())) {
                ...
                if (isSize(token)) {
                    ...
                    font.family = splitBy(tokens.join(' '), /\s*,\s*/).map(
                        unquote,
                    );
                ...
                throw new Error(
                    'Unknown or unsupported font token: ' + token,
                );
            }
        ...
        const markRenderingContextOperator =
            utils.markRenderingContextOperator;
        ...
        const allFonts = _Object
            .keys(fontSalt)
            .map((e) => e.toLowerCase());

        const existFonts = _Object
            .entries(fontSalt)
            .filter((e) => e[1].exists)
        ...
        if (0) {
            console.log(
                `hook font Total fonts present ${existFonts.length}`,
            );
        }
        ...
        const calcNewFont = (font, domNode) => {
            ...
            if (
                font &&
                (font = font.toLowerCase()) !== '10px sans-serif'
            ) {
                if (!domNode) {
                ...
                try {
                    ...
                    try {
                        ...
                        const cs = _window.getComputedStyle.call(
                            null,
                            domNode,
                        );
                        style = cs.fontStyle;
                    ...
                    if (family.length) {
                        ...
                        for (let f of family) {
                            ...
                            if (existFonts.includes(f)) {
                                ...
                                if (!hookFonts.includes(f)) {
                                    ...
                                    if (0) {
                                        console.log(
                                            `hook font new Font ${f}, count: ${hookFonts.length}`,
                                        );
                                    }
                                }
                        ...
                        if (!targetFontFamily) {
                            // Default Font
                            targetFontFamily = _window.getComputedStyle
                                .call(null, domNode)
                        ...
                        let fakeFontConfig = fakeFontConfigMaps.find(
                            (e) =>
                                e.org.style === style &&
                                e.org.weight === weight &&
                                e.org.size === size &&
                        ...
                        if (!fakeFontConfig) {
                            ...
                            if (fontSalt) {
                                ...
                                targetSize =
                                    '' +
                                    (sizeNumericValue + fontSalt.size) +
                                    'px';
                            } else {
                        ...
                        if (0) {
                            console.log(
                                `!!! hook font Original font：${font} New Fonts: ${JSON.stringify(
                                    newFont,
                                )} ${
                                    fCount !== 0
                                        ? " Can't find the font "
        ...
        if (1) {
            ...
            if ('undefined' !== typeof OffscreenCanvas) {
                canvasAndContextClass.push({
                    ...
                    _Canvas_prototype_toDataURL:
                        OffscreenCanvas.prototype.toDataURL,
                    _CanvasRenderingContext2D_prototype_getImageData:
                        OffscreenCanvasRenderingContext2D.prototype
                            .getImageData,
                });

                // noinspection JSUnusedAssignment
                _OffscreenCanvas_prototype_getContext =
                    OffscreenCanvas.prototype.getContext;
            }
            ...
            if ('undefined' !== typeof HTMLCanvasElement) {
                ...
                // noinspection JSUnusedAssignment
                _HTMLCanvasElement_prototype_getContext =
                    HTMLCanvasElement.prototype.getContext;
            }
            ...
            for (const {
                ...
                utils.replaceGetterWithProxy(
                    _CanvasRenderingContext2D.prototype,
                    'font',
                    {
                        apply(target, thisArg, args) {
                            // Current font
                            const contextIndex =
                                markRenderingContextOperator(
                                    thisArg,
                                    'font',
                                );
                ...
                utils.replaceSetterWithProxy(
                    ...
                    'font',
                    {
                        apply(target, thisArg, args) {
                            ...
                            if (0) {
                                console.log(
                                    `!!! h00k context:${contextIndex} set font:${Array.from(
                                        args,
                                    ).join('|')}`,
                                );
                            }
        ...
        if (1) {
            ...
            const hookStyle = (domNode) => {
                let hookConfig = needsToHookStyles.find(
                    (e) => e.domNode === domNode,
                );
                ...
                if (!hookConfig) {
                    ...
                    const styleDeclaration = _Reflect.apply(
                        utils.cache.Descriptor.HTMLElement.prototype.style
                            .get,
                        domNode,
                        [],
                    );
                    ...
                    const handler = {
                        get: (target, property, receiver) => {
                            const orgResult = _Reflect.get(
                                utils.getProxyTarget(target),
                                property,
                            );
                            let result;
                            ...
                            if (
                                ...
                                if (hookAttribs.includes(property)) {
                                    result =
                                        hookConfig.userSettings[property];
                                } else {
                                    result = orgResult;
                                }
                            } else {
                                ...
                                result =
                                    utils.getProxyTarget(target)[property];
                            }
             ...
             * @returns boolean
             */
            const handleUserSetFontStyle = (
                cb,
                styleDeclaration,
                attr,
                value,
            ) => {
                const hookConfig = needsToHookStyles.find(
                    (e) => e.styleDeclaration === styleDeclaration,
                );
                if (hookConfig && attr && value) {
                    ...
                    if (0) {
                        console.log(
                            `hook font Set Properties ${attr} ${value}`,
                        );
                    }
                    ...
                    if (
                        !targetAttr &&
                        !hookAttribBlackList.includes(attr)
                    ) {
                        ...
                        for (let n = 0; n < pseudos.length; ++n) {
                            ...
                            cs[n] = _window.getComputedStyle.call(
                                null,
                                domNode,
                                pseudo,
                            );
                        ...
                        for (let n = 0; n < pseudos.length; ++n) {
                            for (const attr of hookAttribs) {
                                if (
                                    csBefore[n][attr] !== csAfter[n][attr]
                                ) {
                    ...
                    if (targetAttr) {
                        ...
                        if (targetAttr === 'font') {
                            ...
                            try {
                                ...
                                style =
                                    hookConfig.userSettings.fontStyle ||
                                    hookConfig.computedStyles.fontStyle;
                                weight =
                                    hookConfig.userSettings.fontWeight ||
                                    hookConfig.computedStyles.fontWeight;
                                size =
                                    hookConfig.userSettings.fontSize ||
                                    hookConfig.computedStyles.fontSize;
                                variant =
                                    hookConfig.userSettings.fontVariant ||
                                    hookConfig.computedStyles.fontVariant;
                                stretch =
                                    hookConfig.userSettings.fontStretch ||
                                    hookConfig.computedStyles.fontStretch;
                                lineHeight =
                                    hookConfig.userSettings.lineHeight ||
                                    hookConfig.computedStyles.lineHeight;
                            }
                        ...
                        const orgFont = [
                            ...
                            hookConfig.userSettings.fontSize ||
                                hookConfig.computedStyles.fontSize,
                            hookConfig.userSettings.fontFamily ||
                                hookConfig.computedStyles.fontFamily,
                        ]
                            .join(' ')
                            .trim();
                        ...
                        if (newFont) {
                            ...
                            // noinspection JSUnresolvedVariable
                            const setPropertyCaller =
                                utils.cache.Descriptor.CSSStyleDeclaration
                                    .prototype.setProperty.value;

                            // Iterate over the content to be written
                            for (const [attr, destAttr] of Object.entries(
                                finallyAttribsMap,
                            )) {
                                setPropertyCaller.call(
                                    styleDeclaration,
                                    attr,
                                    newFont[destAttr],
                                    'important',
            ...
            if ('undefined' !== typeof CSSStyleDeclaration) {
                const _CSSStyleDeclaration_prototype_setProperty =
                    ...
                    'setProperty',
                    {
                        apply(target, thisArg, args) {
                            const handle = handleUserSetFontStyle(
                                () => {
                                    return _CSSStyleDeclaration_prototype_setProperty.apply(
                                        utils.getProxyTarget(thisArg),
                                        args,
                                    );
                                    // return _Reflect.apply(target, thisArg, args);
                                },
        ...
        if ('undefined' !== utils.cache.global.FontFace) {
            ...
            utils.replaceWithProxy(utils.cache.global, 'FontFace', {
                construct(target, args) {
                    if (0) {
                        console.log(
                            'hook font fontFace constructor called',
                            args,
                        );
                    }
                    ...
                    if (matches && matches.length > 1) {
                        ...
                        if (allFonts.includes(fontFamily)) {
                            if (0) {
                                console.log(
                                    `hook FontFace font ${fontFamily}`,
                                );
                            }

                            fontFaceConfigCache.push({
                                fontFace: result,
                                args,
                                fontFamily,
                            });
            ...
            utils.replaceWithProxy(FontFace.prototype, 'load', {
                async apply(target, thisArg, args) {
                    ...
                    try {
                        // noinspection JSUnresolvedVariable
                        result =
                            await utils.cache.Descriptor.FontFace.prototype.load.value.call(
                                thisArg,
                            ); //_Reflect.apply(target, thisArg, args);
                    } catch (ex) {
                        err = ex;
                    ...
                    if (fontFaceConfig) {
                        ...
                        const fontExists = existFonts.includes(
                            fontFaceConfig.fontFamily,
                        );
                        if (fontExists) {
        ...
        if (
            ...
            'undefined' !== typeof globalThis.fonts
        ) {
            const _FontFaceSet_prototype = _Object.getPrototypeOf(
                globalThis.fonts,
            );
