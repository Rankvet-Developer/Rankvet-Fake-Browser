...
class Plugin extends PuppeteerExtraPlugin {
    ...
    mainFunction = (utils, { chromeMajorVersion, fakePlugins }) => {
        ...
        const pluginsData =
            chromeMajorVersion > 93 ? kPluginsGreaterThen93 : fakePlugins;
        ...
        const nativeMimeTypeArray = _Object.create(
        ...
        for (const pluginData of pluginsData.plugins) {
            const { name, filename, description, __mimeTypes } =
                pluginData;
            ...
            const nativePlugin = new Proxy(nativePluginInner, {
                get: (target, property, receiver) => {
                    ...
                    let mimeType = null;
                    const isInteger =
                        property && Number.isInteger(Number(property));

                    if (isInteger) {
                        const mimeIndex =
                            Number(property) % Math.pow(2, 32);
                    ...
                    if (mimeType) {
                        ...
                        const nativeMimeType = makeNativeMimeType(
                            mimeType,
                            nativePlugin,
                        );
                        return nativeMimeType;
        ...
        for (const mimeTypeData of pluginsData.mimeTypes) {
            const { type, suffixes, description, __pluginName } =
                mimeTypeData;
        ...
        utils.replaceWithProxy(PluginArray.prototype, 'item', {
            apply(target, thisArg, args) {
                ...
                if (thisArg === nativePluginArray) {
                    const isInteger =
                        args[0] && Number.isInteger(Number(args[0]));
                    const index = isInteger
                        ? Number(args[0]) % Math.pow(2, 32)
                        : 0;
        ...
        utils.replaceWithProxy(PluginArray.prototype, Symbol.iterator, {
            apply(target, thisArg, args) {
                ...
                if (thisArg === nativePluginArray) {
                    const nativePluginObjs = pluginCorrs.map(
                        (e) => e.nativePlugin,
                    );
                    return nativePluginObjs[Symbol.iterator].bind(
                        nativePluginObjs,
                    )();
                }
        ...
        utils.replaceWithProxy(MimeTypeArray.prototype, 'namedItem', {
            apply(target, thisArg, args) {
                ...
                if (thisArg === nativeMimeTypeArray) {
                    const name = args[0];
                    const mimeTypeCorr = mimeTypeCorrs.find(
                        (e) => e.mimeTypeData.name === name,
                    );
        ...
        utils.replaceWithProxy(MimeTypeArray.prototype, Symbol.iterator, {
            apply(target, thisArg, args) {
                ...
                if (thisArg === nativeMimeTypeArray) {
                    const nativeMimeTypeObjs = mimeTypeCorrs.map(
                        (e) => e.nativeMimeType,
                    );
                    return nativeMimeTypeObjs[Symbol.iterator].bind(
                        nativeMimeTypeObjs,
                    )();
                }
        ...
        utils.replaceWithProxy(Plugin.prototype, 'item', {
            apply(target, thisArg, args) {
                ...
                if (pluginCorr) {
                    ...
                    if (
                        index < 0 ||
                        index >= pluginCorr.pluginData.__mimeTypes.length
                    ) {
                        return null;
                    }

                    const mimeType =
                        pluginCorr.pluginData.__mimeTypes[index];
                    const nativeMimeType = makeNativeMimeType(
                        mimeType,
                        thisArg,
                    );
        ...
        utils.replaceWithProxy(Plugin.prototype, 'namedItem', {
            apply(target, thisArg, args) {
                ...
                if (pluginCorr) {
                    ...
                    if (
                        !pluginCorr.pluginData.__mimeTypes.includes(
                            mimeType,
                        )
                    ) {
                        return null;
        ...
        utils.replaceWithProxy(Plugin.prototype, Symbol.iterator, {
            apply(target, thisArg, args) {
                ...
                if (pluginCorr) {
                    ...
                    for (const mimeType of pluginCorr.pluginData
                        .__mimeTypes) {
                        const nativeMimeType = makeNativeMimeType(
                            mimeType,
                            thisArg,
                    ...
                    return nativeMimeTypes[Symbol.iterator].bind(
                        nativeMimeTypes,
                    )();
                }
