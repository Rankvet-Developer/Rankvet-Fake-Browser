...
class Plugin extends PuppeteerExtraPlugin {
    ...
    getOverride = async (browser, opts) => {
        ...
        // console.log('useragent opts is ', opts);
        const orgVersion = chromeVersion(orgUA);
        const fakeVersion = chromeVersion(opts.fakeDD.navigator.userAgent);

        opts.fakeDD.navigator.userAgent =
            opts.fakeDD.navigator.userAgent.replace(
                fakeVersion,
                orgVersion,
            );
        ...
        if (this._headless) {
            override.acceptLanguage =
                opts.fakeDD.acceptLanguage || 'en-US,en';
        }
    ...
    async onPageCreated(page) {
        ...
        try {
            ...
            await page
                ._client()
                .send('Network.setUserAgentOverride', override);
        } catch (ex) {
            console.warn(
                'Network.setUserAgentOverride CDPSession Error',
                ex,
                        ...
                        if (args && args[0] && args[0].length) {
                            for (const n of args[0]) {
                                if (n in override.userAgentMetadata) {
                                    result[n] =
                                        override.userAgentMetadata[n];
                                } else if (n === 'uaFullVersion') {
                                    result[n] =
                                        override.userAgentMetadata.fullVersion;
                                }
                            }
                        }
        ...
        return [
            {
                ...
                value: {
                    intl: {
                        accept_languages: this.opts.locale || 'en-US,en',
                    },
                },
            },
        ];
