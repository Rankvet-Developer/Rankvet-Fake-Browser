...
class Plugin extends PuppeteerExtraPlugin {
    ...
    get name() {
        return 'evasions/sourceurl';
    }

    async onPageCreated(page) {
        const client = await page.target().createCDPSession();

        if (!page || !client || !client.send) {
            this.debug('Warning, missing properties to intercept CDP.', {
                page,
            });
            return;
        }
        ...
        client.send = (function (originalMethod, context) {
            return async function () {
                ...
                const next = async () => {
                    try {
                        return await originalMethod.apply(context, [
                            method,
                            paramArgs,
                        ]);
                    } catch (error) {
                ...
                if (
                    ...
                    !paramArgs[methodsToPatch[method]]
                ) {
                    return next();
                }
