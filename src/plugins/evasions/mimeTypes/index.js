...
class Plugin extends PuppeteerExtraPlugin {
    ...
    mainFunction = (utils, fakeMimeTypes) => {
        ...
        utils.replaceWithProxy(HTMLMediaElement.prototype, 'canPlayType', {
            apply: function (target, thisArg, args) {
                ...
                if (trimmedType.endsWith(trimmedType)) {
                    trimmedType = trimmedType.substr(
                        0,
                        trimmedType.length - 1,
                    );
                }
                ...
                if (
                    ...
                    (thisArg.constructor.name === 'HTMLVideoElement' &&
                        !trimmedType.startsWith('video')) ||
                    (thisArg.constructor.name === 'HTMLAudioElement' &&
                        !trimmedType.startsWith('audio'))
                ) {
                    return orgResult;
                }

                const mimeType = fakeMimeTypes.find(
                    (e) => e.mimeType === trimmedType,
                );
                if (mimeType) {
        ...
        utils.replaceWithProxy(MediaSource, 'isTypeSupported', {
            apply: function (target, thisArg, args) {
                const orgResult = _Reflect.apply(target, thisArg, args);
        ...
        if ('undefined' !== typeof MediaRecorder) {
            utils.replaceWithProxy(MediaRecorder, 'isTypeSupported', {
                apply: function (target, thisArg, args) {
                    const orgResult = _Reflect.apply(
                        target,
                        thisArg,
                        args,
                    );
