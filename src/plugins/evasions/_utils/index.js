    ...
    OffscreenCanvas.prototype.constructor.__$cache = utils.cache = {
        ...
        Reflect: {
            ...
            getOwnPropertyDescriptor:
                Reflect.getOwnPropertyDescriptor.bind(Reflect),
    ...
    const toStringProxy = utils.newProxyInstance(
        ...
        utils.stripProxyFromErrors({
            apply: function (target, thisArg, args) {
                ...
                // toStringPatch
                const toStringPatchObj =
                    utils.variables.toStringPatchObjs.find(
                        (e) => e.obj === thisArg,
                    );
                ...
                if (toStringPatchObj) {
                    ...
                    return (
                        toStringPatchObj.str ||
                        utils.makeNativeString(toStringPatchObj.obj.name)
                    );
                }

                // toStringRedirect
                const toStringRedirectObj =
                    utils.variables.toStringRedirectObjs.find(
...
utils.removeTempVariables = () => {
    const tmpVarNames = Object.getOwnPropertyNames(
...
utils.patchError = (err, trap) => {
    ...
    let lineIndex = stackLines.findIndex((e) => {
        const matches = e.match(
            /Object\.ɵɵɵɵnewHandler\.<computed> \[as (.*)]/,
        );
    ...
    for (let n = lineIndex - 1; n >= 0; --n) {
        ...
        if (
            utils.intersectionSet(lineNums, fbCodeStackLineNumbers).size >
            0
        ) {
...
utils.replaceGetterWithProxy = (obj, propName, handler) => {
    const desc = utils.cache.Object.getOwnPropertyDescriptor(
        obj,
        propName,
    );
    if (desc) {
        const fn = utils.cache.Object.getOwnPropertyDescriptor(
            obj,
            propName,
        ).get;
...
utils.mockGetterWithProxy = (
    ...
    utils.patchToString(
        proxyObj,
        `function get ${propName}() { [native code] }`,
    );
...
utils.mockSetterWithProxy = (
    ...
    utils.patchToString(
        proxyObj,
        `function set ${propName}() { [native code] }`,
    );
...
utils.markRenderingContextOperator = (context, operatorName) => {
    ...
    if (result >= 0) {
        const operators =
            utils.variables.renderingContextWithOperators[result];
        if (operators) {
...
utils.findRenderingContextIndex = (canvas) => {
    ...
    for (let contextId of contextIds) {
        ...
        if (
            utils.cache.Object.getPrototypeOf(canvas) ===
            OffscreenCanvas.prototype
        ) {
            context =
                utils.cache.OffscreenCanvas.prototype.getContext.call(
                    canvas,
                    contextId,
                );
        } else {
            context =
                utils.cache.HTMLCanvasElement.prototype.getContext.call(
                    canvas,
                    contextId,
                );
        }
