...
utils._preloadCache = () => {
    ...
    OffscreenCanvas.prototype.constructor.__$cache = utils.cache = {
        ...
        window: {
            getComputedStyle: 'undefined' !== typeof window &&
                window.getComputedStyle.bind(window),
            eval: 'undefined' !== typeof window
                ? window.eval.bind(window)
                : globalThis
                    ? globalThis.eval.bind(globalThis)
                    : undefined,
            navigator: 'undefined' !== typeof window
                ? window.navigator
                : globalThis
        ...
        OffscreenCanvas: {
            prototype: {
                getContext: 'undefined' !== typeof OffscreenCanvas &&
                    OffscreenCanvas.prototype.getContext,
            },
        },
        HTMLCanvasElement: {
            prototype: {
                getContext: 'undefined' !== typeof HTMLCanvasElement &&
...
utils._hookObjectPrototype = () => {
    ...
    utils.objHooked =
        OffscreenCanvas.prototype.constructor.__$objHooked = true;
    ...
    const toStringProxy = utils.newProxyInstance(Function.prototype.toString, utils.stripProxyFromErrors({
        apply: function (target, thisArg, args) {
            ...
            if (toStringPatchObj) {
                ...
                return (toStringPatchObj.str ||
                    utils.makeNativeString(toStringPatchObj.obj.name));
            }
            // toStringRedirect
            ...
            const hasSameProto = _Object
                .getPrototypeOf(Function.prototype.toString)
...
utils.removeTempVariables = () => {
    const tmpVarNames = Object.getOwnPropertyNames(OffscreenCanvas.prototype.constructor).filter((e) => e.startsWith('__$'));
...
utils.patchError = (err, trap) => {
    ...
    for (let n = lineIndex - 1; n >= 0; --n) {
        ...
        if (line.includes(`at new ɵɵɵɵPromise (eval at <anonymous>`)) {
            ...
            // 4.1
            if (stackLines[n - 1] &&
                stackLines[n - 1].includes(`at new Promise (<anonymous>)`)) {
                --n;
                stackLines.splice(n, 1);
            }
            continue;
        ...
        if (utils.intersectionSet(lineNums, fbCodeStackLineNumbers).size >
            0) {
...
utils.stripProxyFromErrors = (handler = {}) => {
    ...
    const ɵɵɵɵnewHandler = {
        setPrototypeOf: function (target, proto) {
            ...
            if (_Object.getPrototypeOf(target) ===
                _Object.getPrototypeOf(proto)) {
...
utils.replaceProperty = (obj, propName, descriptorOverrides = {}) => {
    ...
    //     }
    // }
    return _Object.defineProperty(obj, propName, Object.assign(Object.assign({}, descriptors), descriptorOverrides));
};
/**
...
utils.mockWithProxy = (obj, propName, pseudoTarget, descriptorOverrides, handler) => {
    ...
    const proxyObj = pseudoTarget
        ...
        : utils.stripProxyFromErrors(handler);
    utils.replaceProperty(obj, propName, Object.assign(Object.assign({}, descriptorOverrides), { value: proxyObj }));
...
utils.mockGetterWithProxy = (obj, propName, pseudoTarget, descriptorOverrides, handler) => {
    ...
    const proxyObj = pseudoTarget
        ? utils.newProxyInstance(pseudoTarget, utils.stripProxyFromErrors(handler))
        : utils.stripProxyFromErrors(handler);
    utils.replaceProperty(obj, propName, Object.assign(Object.assign({}, descriptorOverrides), { get: proxyObj }));
...
utils.mockSetterWithProxy = (obj, propName, pseudoTarget, descriptorOverrides, handler) => {
    ...
    const proxyObj = pseudoTarget
        ? utils.newProxyInstance(pseudoTarget, utils.stripProxyFromErrors(handler))
        : utils.stripProxyFromErrors(handler);
    utils.replaceProperty(obj, propName, Object.assign(Object.assign({}, descriptorOverrides), { set: proxyObj }));
...
utils.isSequence = (obj) => {
    const _Object = utils.cache.Object;
    let desc = null;
    for (; obj && !!(desc = _Object.getOwnPropertyDescriptors(obj));) {
        if (desc.forEach) {
            return true;
        }
...
utils.getCurrentScriptPath = () => {
    ...
    let rExtractUri = /(?:http|https|file):\/\/.*?\/.+?\.js/, absPath = rExtractUri.exec(stack);
    if (!absPath) {
...
utils.makePseudoClass = (root, name, pseudoTarget, parentClass) => {
    const _Object = utils.cache.Object;
    const result = new Proxy(pseudoTarget ||
        function () {
...
utils.findRenderingContextIndex = (canvas) => {
    ...
    for (let contextId of contextIds) {
        let context = null;
        if (utils.cache.Object.getPrototypeOf(canvas) ===
            OffscreenCanvas.prototype) {
            context =
                utils.cache.OffscreenCanvas.prototype.getContext.call(canvas, contextId);
        }
        else {
            context =
                utils.cache.HTMLCanvasElement.prototype.getContext.call(canvas, contextId);
        }
