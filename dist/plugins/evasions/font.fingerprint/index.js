...
class Plugin extends PuppeteerExtraPlugin {
    constructor(opts = {}) {
        super(opts);
        this.mainFunction = (utils, fontSalt) => {
            ...
            let hookCount = 0;
            // All lowercase
            const fontSaltWithLowerCaseName = _Object.fromEntries(_Object.entries(fontSalt).map(e => {
                e[0] = e[0].toLowerCase();
                return e;
            }));
            ...
            if (1) {
                ...
                for (const { _Canvas, _CanvasRenderingContext2D } of canvasAndContextClass) {
                    ...
                    utils.replaceWithProxy(CSSStyleDeclaration.prototype, 'setProperty', {
                        apply(target, thisArg, args) {
                            const handle = handleUserSetFontStyle(() => {
                                ...
                                // return _Reflect.apply(target, thisArg, args);
                            }, thisArg, args && args[0], args && args[1]);
                            if (!handle) {
                                // _Reflect.apply(target, thisArg, args);
                                return _CSSStyleDeclaration_prototype_setProperty.apply(utils.getProxyTarget(thisArg), args);
                            }
