...
class Plugin extends PuppeteerExtraPlugin {
    constructor(opts = {}) {
        super(opts);
        this.mainFunction = (utils, canvasSalt) => {
            ...
            const getContextImageUint8Data = (context) => {
                ...
                if ('undefined' !== typeof CanvasRenderingContext2D
                    ...
                    result = _CanvasRenderingContext2D_prototype_getImageData.call(context, 0, 0, context.canvas.width, context.canvas.height).data;
                }
                else if ('undefined' !== typeof OffscreenCanvasRenderingContext2D
                    && contextPrototype === OffscreenCanvasRenderingContext2D.prototype) {
                    // OffscreenCanvasRenderingContext2D
                    result = _OffscreenCanvasRenderingContext2D_prototype_getImageData.call(context, 0, 0, context.canvas.width, context.canvas.height).data;
                }
                else if ('undefined' !== typeof WebGLRenderingContext
                    ...
                    _WebGLRenderingContext_prototype_readPixels.call(context, 0, 0, context.drawingBufferWidth, context.drawingBufferHeight, context.RGBA, context.UNSIGNED_BYTE, result);
                }
                else if ('undefined' !== typeof WebGL2RenderingContext
                    && contextPrototype === WebGL2RenderingContext.prototype) {
                    // WebGL2RenderingContext
                    result = new Uint8ClampedArray(context.drawingBufferWidth * context.drawingBufferHeight * 4);
                    _WebGL2RenderingContext_prototype_readPixels.call(context, 0, 0, context.drawingBufferWidth, context.drawingBufferHeight, context.RGBA, context.UNSIGNED_BYTE, result);
                }
                return result;
            };
            // noinspection JSUnusedLocalSymbols
            for (const { _Canvas, _CanvasRenderingContext2D, _Canvas_prototype_getContext, _Canvas_prototype_toDataURL, } of classes) {
                // noinspection JSUnusedLocalSymbols
                ...
                const getNoisifyCanvas = (canvas) => {
                    const { context: originalContext, contextIndex, } = utils.findRenderingContextIndex(canvas);
                    if (contextIndex < 0) {
                        return canvas;
                    }
                    ...
                    if (!((context.contextId === 'webgl' || context.contextId === 'experimental-webgl')
                    ...
                    // }) + ' noise !');
                    // Create a new canvas
                    let canvasWithNoise = _Object.getPrototypeOf(canvas) === (('undefined' !== typeof OffscreenCanvas) && OffscreenCanvas.prototype)
                    ...
                    let newContext = _Object.getPrototypeOf(canvas) === (('undefined' !== typeof OffscreenCanvas) && OffscreenCanvas.prototype)
                    ...
                    for (let y = 0; y < canvasHeight - 1; y += 2) {
                        for (let x = 0; x < canvasWidth - 1; x += 2) {
                            ...
                            // bottom right
                            // 4 pixel points
                            const p00 = imageUint8DataOriginal[pos], p01 = imageUint8DataOriginal[pos + 1];
                            const p10 = imageUint8DataOriginal[pos + canvasWidth], p11 = imageUint8DataOriginal[pos + canvasWidth + 1];
